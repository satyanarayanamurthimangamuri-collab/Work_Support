...
export default function WorkWithTeam() {
  ...
  return <main>
    ...
    <section className="border-y border-border bg-white"><div className="mx-auto max-w-8xl px-6 py-20 lg:px-10 lg:py-24"><SectionHeading title="Support That Fits the Way Your Team Works" description="Every team has different goals, workflows and challenges. Work Support can be structured around what your team actually needs." /><div className="grid grid-cols-1 gap-8 md:grid-cols-3 md:gap-0">{FIT_STEPS.map(([number, title, description], index) => <Reveal key={number} index={index} className="relative md:px-8 first:md:pl-0 last:md:pr-0"><article className="relative h-full border-border md:border-l md:pl-8 first:md:border-l-0 first:md:pl-0"><span className="font-mono text-[12px] font-semibold text-blue">{number}</span><h3 className="mt-3 text-[18px] font-bold text-navy">{title}</h3><p className="mt-2 text-[14px] leading-[1.7] text-text-muted">{description}</p></article></Reveal>)}</div></div></section>
